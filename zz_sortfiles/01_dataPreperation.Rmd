...
################
## load additional self written functions
setwd("functions_additional")
...
setwd("..")
rm(f) # remove object from environment
```
...
The file include all analysis code for the published article XXXXX. In the article a multi-method approach was applied, whereby two modes of data collection - Cognitive-Affective Maps (CAM) and surveys - in accordance with two statistical procedures - graph theory [e.g., @newmanNetworksIntroduction2018] and Latent Variable Models [e.g., @skrondalGeneralizedLatentVariable2004] - have been combined. 

This file contains the pre-processing step (clean, transform data), whereby the file "02_analysis" the analysis step (test hypotheses and exploratory analyses), which follows the classical data-analysis pipeline [see @pengArtDataScience2016; @wickhamDataScienceImport2017].
...
Define the *global variables* to increase the compilation time of the document (feel free to change):

```{r globalVariables, include=TRUE}
global_saveFakeCAMs = FALSE # TRUE / FALSE


global_removeFakeCAMs = TRUE # ! keep to TRUE

global_removeSpam = FALSE # TRUE / FALSE
...
```{r prepareData_survey, include=TRUE}
################
...
rm(remove_vars)
################
# prepare data (everything is a string)
################
## t1 START ##
#> part 1
# str(survey_p1_t1)
...
```{r prepareData_CAM, include=TRUE}
################
# create CAM files
################
## t1
...
```{r removeTestIDandMissing_cleanData, include=TRUE}
################
# remove test IDs
################
## t1
...
sum(survey_p1_t1$attention.check != "Tennisspielen", na.rm=T)
survey_p1_t1 <- survey_p1_t1[survey_p1_t1$attention.check == "Tennisspielen", ]
sum(survey_p1_t2$attention.check != "Tennisspielen", na.rm=T)
survey_p1_t2 <- survey_p1_t2[survey_p1_t2$attention.check == "Tennisspielen", ]
sum(survey_p1_t2_controlGroup$attention.check != "Tennisspielen", na.rm=T)
...
Before data is cleaned according to different criteria some descriptive statistics:

```{r descriptiveStats_cleanData, include=TRUE}
## t1
nrow(survey_p1_t1)
nrow(survey_p2_t1)
## t2
...
```
`r nrow(survey_p2_t1)` recorded completes in wave 1
...
```{r ipFilter_cleanData, include=TRUE}
...
overview of removed data in case of "if IPs are greater / equal then 2, 3, 4" criterion: 


```{r check_ipFilter_cleanData, include=TRUE}
...
> we need to decide to remove participants we IPs are greater / equal then 2 or 3


### draw geolocations of participants


```{r drawGeolocations, include=TRUE}
# ???
## t1
...
### problem of multiple participants

Exemplify the problem of multiple IPs for the first part of first measurment point: 

```{r problem_multipleIPs, include=FALSE}
## if "character(0)" no more multiple IDs
multiIDs <- names(table(survey_p1_t1$participantID))[table(survey_p1_t1$participantID) >= 2]

if(!identical(multiIDs, character(0))){
  length(multiIDs)
  survey_p1_t1[survey_p1_t1$participantID == multiIDs[1],]
}
...
```{r dependencyFilters, include=FALSE}
...
```{r spamFiler_remove_cleanData, include=TRUE}
