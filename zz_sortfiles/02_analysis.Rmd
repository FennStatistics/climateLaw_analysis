...
survey_t2_CAM <- survey_t2_CAM %>% filter(gender=="männlich" | gender=="weiblich")
...
survey_CAM$partyIdentification <- factor(survey_CAM$partyIdentification,
                                                           levels=c("Grüne", "SP", "GLP", "Die Mitte", "FDP","SVP", "andere Partei", "Keine") )
...
ggplot(survey_CAM_party, aes(x=partyIdentification, y=perc, fill=partyIdentification)) +
  ...
  scale_fill_manual(values= c("#84B414", "#FF0000", "#000000", "#FF9B00", "#074EA1",  "#72BE44", "grey", "grey" )) +
      theme(axis.title  = element_text(size = 24, family = 'Arial', color="black"), 
          axis.text.y = element_text(size = 24, family = 'Arial', color="black"),
          axis.text.x = element_text(size=24,family = 'Arial', color="black"),
          strip.text = element_text(size=24, family = 'Arial', color="black"),
...
climate_concern_party.vis$partyIdentification <- factor(climate_concern_party.vis$partyIdentification,
                                                           levels=c("Grüne", "SP", "GLP", "Die Mitte", "FDP","SVP") )
...
media.engagement.party.vis$partyIdentification <- factor(media.engagement.party.vis$partyIdentification,
                                                           levels=c("Grüne", "SP", "GLP", "Die Mitte", "FDP","SVP"),
                                                          labels=c("Grüne", "SP", "GLP", "Die Mitte", "FDP","SVP"))
...
survey_CAM$partyIdentification <- factor(survey_CAM$partyIdentification,
                                                           levels=c("Grüne", "SP", "GLP", "Die Mitte", "FDP","SVP", "Keine", "andere Partei") )
...
#survey_t1_CAM.rep <- merge(survey_t1_CAM, survey_experimentalGroup_CAM.long.rep.ex, by="participantID")
...
```{r}
survey_CAM2 <- survey_CAM %>% filter(!wave=="wave2_control")
survey_CAM.rep <- merge(survey_CAM2, survey_experimentalGroup_CAM.long.rep.ex, by="participantID")
survey_CAM.rep$partyIdentification <- factor(survey_CAM.rep$partyIdentification, levels=c("SVP", "FDP", "Die Mitte","GLP", "SP","Grüne", "andere Partei", "Keine"))


survey_CAM.rep.party <- survey_CAM.rep %>% filter(!c(partyIdentification=="Keine" | partyIdentification=="andere Partei"))
survey_CAM.rep.party$partyIdentification <- factor(survey_CAM.rep.party$partyIdentification, levels=c("SVP", "FDP", "Die Mitte","GLP", "SP","Grüne"))

model_H2_t1_nodes_law <- lmer(num_nodes_macro ~ ratingLaw  + partyIdentification +
                              climate_concern +
                                age + gender + education +
                                wave +
                                (1|participantID),
                              data=survey_CAM.rep)
tab_model(model_H2_t1_nodes_law)
```
...
## exclude unrepresentative CAMs

survey_experimentalGroup_CAM.long.rep.ex <- survey_experimentalGroup_CAM.long %>% select(participantID, representative_t1, representative_t2) %>% filter(!c(representative_t1=="etwas unrepräsentativ" | representative_t2=="etwas unrepräsentativ" | representative_t2=="sehr unrepräsentativ"  ))
survey_experimentalGroup_CAM.long.rep.ex <- survey_experimentalGroup_CAM.long.rep.ex %>% select(participantID)
survey_experimentalGroup_CAM.long.rep.ex <- unique(survey_experimentalGroup_CAM.long.rep.ex)

survey_experimentalGroup_CAM.long.rep <- merge(survey_experimentalGroup_CAM.long, survey_experimentalGroup_CAM.long.rep.ex, by="participantID")
...
# if we exclude participants that said they feel their mental models are (very) unrepresentative it is significant

```{r}
model_H6_macro_media <- lmer(mean_valence_macro ~ wave + 
                               intendedVote*media_engagement_rel +
                         climate_concern_rel  +
                         age + gender + education + 
                   (1|participantID),
                  data=survey_experimentalGroup_CAM.long.rep)
...
model_H6_macro_3fach <- lmer(mean_valence_macro ~ wave*media_engagement_rel*intendedVote  +  
                            climate_concern_rel  + media_engagement_rel +
                            age + gender + education + 
                            (1|participantID) ,
                        data=survey_experimentalGroup_CAM.long.rep)
tab_model(model_H6_macro_3fach)

plot_model(model_H6_macro_3fach, type="emm", terms=c("media_engagement_rel", "intendedVote", "wave"), colors = c("darkred", "darkgreen") ) +
    theme(axis.title  = element_text(size = 22, family = 'Arial', color="black"), 
          axis.text.y = element_text(size = 20, family = 'Arial', color="black"),
          axis.text.x = element_text(size=20,family = 'Arial', color="black"),
          strip.text = element_text(size=20, family = 'Arial', color="black"),
          legend.text = element_text(size=18, family = 'Arial', color="black"),
          legend.title = element_text(size=20, family = 'Arial', color="black")) + labs(x="relative media engagement")
```
...
predicting vote
=> idea: does adding CAM indicators improve model fit or predictions?
*It seems overall not really ..*

```{r}

survey_experimentalGroup_CAM.long.rep <- survey_experimentalGroup_CAM.long.rep %>% filter(gender=="männlich" | gender=="weiblich")

survey_t1_CAM.long.rep <- survey_experimentalGroup_CAM.long.rep %>% filter(wave=="t1")
survey_t2_CAM.long.rep <- survey_experimentalGroup_CAM.long.rep %>% filter(wave=="t2")

model.vote.predict <- glm(intendedVote ~ age + gender + education + politicalOrientation + 
                              num_nodes_macro + mean_valence_macro +
                            climate_concern ,family="binomial",
                           data= survey_t1_CAM.long.rep)
tab_model(model.vote.predict)

model.vote.predict.wo <- glm(intendedVote ~ age + gender + education + politicalOrientation +
                             climate_concern , family="binomial",
                           data= survey_t1_CAM.long.rep)
tab_model(model.vote.predict.wo)

data.pred.vote.CAM<- data.frame(actual= survey_t1_CAM.long.rep$intendedVote, predicted=predict(model.vote.predict), participantID = survey_t1_CAM.long.rep$participantID)
data.pred.vote.wo<- data.frame(actual= survey_t1_CAM.long.rep$intendedVote, predicted=predict(model.vote.predict.wo), participantID = survey_t1_CAM.long.rep$participantID)

data.predictions.vote <- merge(data.pred.vote.CAM, data.pred.vote.wo, by=c("participantID", "actual"))
data.predictions.vote <- data.predictions.vote %>%
  rename( predicted.CAM=predicted.x,
           predicted.without=predicted.y)
data.predictions.vote.long <- pivot_longer(data.predictions.vote, cols= c(predicted.CAM, predicted.without),names_to = "model", values_to = "prediciton")
data.predictions.vote.long$prediciton.binary <- ifelse(data.predictions.vote.long$prediciton >0 , "yes", "no")

data.predictions.vote.sum <- data.predictions.vote.long %>% select(actual, prediciton.binary, model) %>% group_by(actual, prediciton.binary, model) %>% tally()
ggplot(data.predictions.vote.sum, aes(x=actual, y=n, fill=model, label=n)) + geom_bar(stat="identity") + facet_grid(model ~prediciton.binary) +
    theme_minimal() +
    geom_label()
table(data.predictions.vote.long$prediciton.binary, data.predictions.vote.long$actual,  data.predictions.vote.long$model)
```

predicting rating of the law
```{r}

model.rating.predict <- lm(ratingLaw ~ age + gender + education + politicalOrientation  +
                             climate_concern  + 
                         #    density_macro  +  num_edges_macro +
                             mean_valence_macro ,
                           data= survey_t1_CAM.long.rep)
tab_model(model.rating.predict)

data.pred.rating.CAM<- data.frame(actual= survey_t1_CAM.long.rep$ratingLaw, predicted=predict(model.rating.predict), participantID = survey_t1_CAM.long.rep$participantID)

model.rating.predict.wo <- lm(ratingLaw ~ age + gender + education + politicalOrientation + 
                             climate_concern ,
                         #    mean_valence_macro + num_nodes_macro,
                           data= survey_t1_CAM.long.rep)
tab_model(model.rating.predict.wo)
data.pred.rating.wo <- data.frame(actual= survey_t1_CAM.long.rep$ratingLaw, predicted=predict(model.rating.predict.wo), participantID = survey_t1_CAM.long.rep$participantID)

data.predictions <- merge(data.pred.rating.CAM, data.pred.rating.wo, by=c("participantID", "actual"))
data.predictions <- data.predictions %>%
  rename( predicted.CAM=predicted.x,
           predicted.without=predicted.y)
data.predictions.long <- pivot_longer(data.predictions, cols= c(predicted.CAM, predicted.without),names_to = "model", values_to = "prediciton")

ggplot(data.predictions.long,aes(x=actual, y=prediciton, color=model) ) +
  geom_jitter(width=0.05) + facet_wrap(model ~.) +
  geom_abline(slope=1) +
  theme_minimal()

data.predictions.cor <- data.predictions %>%
    summarize(correlation.without = cor(actual, predicted.without),
              corrleation.with = cor(actual, predicted.CAM, use="complete.obs", method="spearman"))

data.predictions.cor
